...
def convert_condition_list_to_vars( lambda_expr, lambda_expr_list, channel_id):
    ...
    vars['crit3'] = {}
    vars['crit3']['type'] = "lambda"
    vars['crit3']['value'] = lambda_expr
    return vars
...
def parse_expr_list(exp_list, lambda_expr, count, lambda_expr_list, expr_list_keys):
    if isinstance(exp_list, dict):
        ...
        chords_id = get_chords_id_for_variable(exp_list['key'])
        inter = "(\"var\" == '" + str(chords_id) + "') AND (\"value\"" + exp_list['operator'] + str(
            exp_list['val']) + ")"
        lambda_expr_list.append((count,chords_id, inter, exp_list['val']))
        # with key
